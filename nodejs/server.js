...
// Function to get on-call dates from DynamoDB for a specific NTID
async function getOnCallByNTID(ntid) {
    try {
        console.log('Scanning for on-call dates for NTID:', ntid);

        const params = {
            TableName: 'OnCallSchedule',
            FilterExpression: 'contains(ID, :ntid)',
        ...
        const onCallDates = result.Items.map(item => ({
            Start: item.Start.S,
            End: item.End.S,
            CSP: item.CSP.S
        }));

        console.log('On-call dates for NTID:', onCallDates);
        return onCallDates;

    } catch (error) {
        console.error('Error scanning DynamoDB for NTID:', error);
        throw error;
    }
}

// POST route to get on-call dates by NTID
app.post('/getOnCallByNTID', async (req, res) => {
    const { ntid } = req.body;
    try {
        const onCallInfo = await getOnCallByNTID(ntid);
        res.json(onCallInfo);
    } catch (error) {
        console.error("Error fetching on-call dates by NTID:", error);
        res.status(500).send("Internal Server Error");
    }
});

// POST route to get on-call engineers for a specific date
app.post('/getOnCallByDate', async (req, res) => {
    const { date } = req.body;
    try {
        const onCallInfo = await getOnCallEngineers(date);
        res.json(onCallInfo);
    } catch (error) {
        console.error("Error fetching on-call schedule by date:", error);
        res.status(500).send("Internal Server Error");
    }
});

// POST route to get on-call engineers for today or a provided date
